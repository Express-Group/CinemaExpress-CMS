...
class livenow extends CI_Controller{
	...
	public function GetData(){
		$Data=$this->livenow_model->GetArticleData(95 , $this->input->post('search'));
		$Content=array();
		$Fullcontent=array();
	...
	public function PutFile($FilePath,$content){
		$path='/var/www/html/cinemaexpress_app/application/views/LIVENOW/';
		chmod($path ,0777);
		$handle = fopen($path.$FilePath , 'w+') or die("can't open file");
	...
	public function index(){
		...
		$totalRows = $this->db->query("SELECT article.content_id FROM articlemaster as article INNER JOIN articlerelateddata as related ON article.content_id = related.content_id WHERE related.section_id=95 ".$search."")->num_rows();
		$row = ($this->input->get('per_page')!='') ? $this->input->get('per_page') : 0;
		$this->load->library('pagination');
		$config['base_url'] = base_url(folder_name.'/livenow');
	...
	public function formJson($content_id){
		...
		if(count($json['details']) > 0){
			$fullContents = json_encode(utf8ize($json));
			chmod($path ,0777);
			$handle = fopen($path.$fileName , 'w+') or die("can't open file");
			if(flock($handle, LOCK_EX)){
				fwrite($handle, $fullContents);
				flock($handle, LOCK_UN);
			}
			fclose($handle);
			chmod($path ,0755);
			$this->PutFile($fileName,$fullContents);
