...
class Commonwidget extends CI_Controller 
{
	...
	public function GetRelatedArticles(){
		...
		foreach($Result as $Data):
			if($Contenttype==1){
				...
				$Class="col-md-12 relatedarticle-custom";
			}else if($Contenttype==2){
				$Img=$this->formImage($Data->video_image_path);
				$Class="col-md-4 relatedarticle-custom";
			}else if($Contenttype==3){
				$Img=$this->formImage($Data->first_image_path);
				$Class="col-md-4 relatedarticle-custom";
			}
			$Template .='<div class="'.$Class.'">';
			$Template .='<img src="'.$Img.'" class="img-responsive">';
			$Template .='<div class="screen"></div>';
			$Template .='<h4><a href="'.BASEURL.$Data->url.'">'.$Data->title.'</a></h4>';
			$Template .='<div class="descript">'.str_replace(['<p>','</p>'],'',$Data->summary_html).'</div>';
			$Template .='<a  href="'.BASEURL.$Data->url.'" class="btn btn-default click_btn">view more</a>';
			$Template .='</div>';
		endforeach;
		echo $Template;
	}
	public function formImage($ImageFile){
		if($ImageFile!=''){
				$ImageFile=image_url.imagelibrary_image_path.str_replace('original','w600X300',$ImageFile);
		}else{
			$ImageFile=image_url.imagelibrary_image_path.'logo/nie_logo_600X300.jpg';
		} 
		return $ImageFile;
	}
