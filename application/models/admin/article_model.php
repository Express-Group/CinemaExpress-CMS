...

{
	...
	*/
...

	{
		...
		$null_value 		= "NULL";
			...
			if($article_details['LiveArticleDetails']['parent_section_name']=='Reviews' || $article_details['LiveArticleDetails']['section_name']=='Reviews' || $article_details['LiveArticleDetails']['section_name']=='test'):
				$reviewdata = array();
				$reviewdata['content_id'] = $article_id;
				$reviewdata['movie_name'] = $movie_name;
				$reviewdata['movie_director'] = $director_name;
				$reviewdata['movie_cast'] = json_encode($movie_person_name);
				$reviewdata['rating'] = $review_value;
				$reviewdata['user_id'] = USERID;
				$reviewdata['modified_on'] = $article_details['modifiedon'];
				$this->db->insert('review_master' , $reviewdata);
				$this->live_db->insert('review_master' , $reviewdata);
			endif;
			...
			*/
			...
			$article_details['content_id'] = $article_id;
				...
				if($article_details['LiveArticleDetails']['parent_section_name']=='Reviews' || $article_details['LiveArticleDetails']['section_name']=='Reviews' || $article_details['LiveArticleDetails']['section_name']=='test'):
					...
					$reviewdata['user_id'] = USERID;
					$reviewdata['modified_on'] = $article_details['modifiedon'];
					$checkdetails = $this->db->query("SELECT content_id FROM review_master WHERE content_id='".$article_id."'")->num_rows();
					if($checkdetails > 0){
						$this->db->where('content_id' , $article_id);
						$this->db->update('review_master' , $reviewdata);
						$this->live_db->where('content_id' , $article_id);
						$this->live_db->update('review_master' , $reviewdata);
					}else{
						$this->db->insert('review_master' , $reviewdata);
						$this->live_db->insert('review_master' , $reviewdata);
					}
					...
					$this->live_db->where('content_id', $article_id);
					$this->live_db->delete('review_master');
				endif;
	...
	*/
	...
	public function review_details($article_id){
		$this->db->select('review_id,movie_name ,movie_director,movie_cast');
		...
		if($result->num_rows() ==1){
			$data = $result->result();
			return ['movie_name'=>$data[0]->movie_name,'movie_director'=>$data[0]->movie_director,'movie_cast'=>json_decode($data[0]->movie_cast)];
		}else{
			return ['movie_name'=>'','movie_director'=>'','movie_cast'=>array()];
		}
