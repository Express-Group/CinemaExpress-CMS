...
class Widget_config_class extends Widget_model
{
	public function GetRelatedArticles($ContentId,$ContentType){
		...
		$NextArticle=$this->live_db->query("SELECT title,url,summary_html,".$ImageName." FROM  ".$TableName." WHERE section_id='".$Section."' AND content_id >'".$ContentId."' ORDER BY content_id  LIMIT 2 ")->result();
		
		if(count($PrevoiusArticle) < 1){
				$NextArticle=$this->live_db->query("SELECT title,url,summary_html,".$ImageName." FROM  ".$TableName." WHERE section_id='".$Section."' AND content_id >'".$ContentId."' ORDER BY content_id  LIMIT 3 ")->result();
		}
		if(count($NextArticle) ==1){
				$PrevoiusArticle=$this->live_db->query("SELECT title,url,summary_html,".$ImageName." FROM  ".$TableName." WHERE section_id='".$Section."' AND content_id <'".$ContentId."' ORDER BY content_id DESC LIMIT 2 ")->result();
		}
		if(count($NextArticle) ==0){
				$PrevoiusArticle=$this->live_db->query("SELECT title,url,summary_html,".$ImageName." FROM  ".$TableName." WHERE section_id='".$Section."' AND content_id <'".$ContentId."' ORDER BY content_id DESC LIMIT 3 ")->result();
		}
