...
.movie_cast,.movie_cast_button{float:left;width:88%;}
.movie_cast{margin-top:10px;}
</style>
...
<textarea class="ckeditor"  id="article_head_line_id" name="txtArticleHeadLine"><?php echo set_value('txtArticleHeadLine'); ?><?php  if(isset($get_article_details['title'])) echo $get_article_details['title']; ?></textarea>
 ...
 <?php if(isset($section_mapping)) { 
				...
				$section_bool = true;
				 ...
				 $condition = $mapping['Sectionname'] != 'Galleries' && $mapping['Sectionname'] != 'Video' && $mapping['Sectionname'] != 'Audios' &&  $mapping['Sectionname'] != 'Resources' &&  $mapping['Sectionname'] != 'Photos';
...
<section id="examples">
    ...
    <div id="tagform-full"  class="bs-example">
		...
		?>
		<br><br>
		<div class="bs-example" id="reviews">
			<label>Reviews:</label>
			<input type="number" size="3" name="review_value" value="<?php print $review ?>">
		</div>
		<?php
		else:
		?><br><br>
		<div class="bs-example" id="reviews">
			<label>Reviews:</label>
			<input type="number" size="3" name="review_value" >
		</div>
		<?php
			endif;
		?>
		<?php if(isset($review_details['movie_name']) && $review_details['movie_name']!=''){ ?>
		<div class="bs-example movie-review" style="margin-top: 10px;">
			<label>Movie Name:</label>
			<input type="text"  name="movie_name" value="<?php echo $review_details['movie_name']; ?>">
		</div>
		<?php }else{ ?>
		<div class="bs-example movie-review">
			<label>Movie Name:</label>
			<input type="text"  name="movie_name">
		</div>
		<?php } ?>
		<?php if(isset($review_details['movie_director']) && $review_details['movie_director']!=''){ ?>
		<div class="bs-example movie-review" style="margin-top: 10px;">
			<label>Director Name:</label>
			<input type="text"  name="director_name" value="<?php echo $review_details['movie_director']; ?>">
		</div>
		<?php }else{ ?>
		<div class="bs-example movie-review" style="margin-top:15px;">
			<label>Director Name:</label>
			<input type="text"  name="director_name">
		</div>
		<?php } ?>
		<?php if(isset($review_details['movie_cast']) && count($review_details['movie_cast'])>0){ ?>
			<div class="bs-example movie-review" style="margin-top: 10px;">
				<label>Movie Cast:</label>
				<?php
				for($m=0;$m<count($review_details['movie_cast']);$m++){
					echo '<div class="movie_cast">';
					echo '<input type="text" placeholder="Enter Name" value="'.$review_details['movie_cast'][$m].'"  name="movie_person_name[]">';
					echo '</div>';
				}
				?>
				<div class="movie_cast_button" style="margin-top: 10px;">
					<button type="button" class="btn btn-primary cast" button-type="+">+</button>
					<button type="button" class="btn btn-primary cast" button-type="-">-</button>
				</div>
			</div>
		<?php }else{ ?>
			<div class="bs-example movie-review"  style="margin-top:15px;display:none;">
				<label>Movie Cast:</label>
				<div class="movie_cast" style="display:none;">
					<input type="text" placeholder="Enter Name"  name="movie_person_name[]">
				</div>
				<div class="movie_cast_button" style="margin-top:3%;display:none;">
					<button type="button" class="btn btn-primary cast" button-type="+">+</button>
					<button type="button" class="btn btn-primary cast" button-type="-">-</button>
				</div>
			</div>
		...
		<script>
			$('.cast').on('click',function(e){
				...
				if(type=='+'){
					var a = '<div class="movie_cast"><input type="text" placeholder="Enter Name"  name="movie_person_name[]"></div>';
					$('.movie_cast').eq($('.movie_cast').length - 1).after(a);
				}else{
					if($('.movie_cast').length > 1)
						$('.movie_cast').eq($('.movie_cast').length - 1).remove();
				}
			});
		</script>
		...
		<div class="example example_markup">
			<div id="seo_tagform_full"  class="bs-example1">
				<?php if(isset($seo_tags)) { 
					...
					<input type="text" name="seo_txtTags[]" id="tag_edit1"  value="<?php echo $seo_tags[$k]; ?>" class="tag"/>
				<?php }  } else { ?>
					<input type="text" name="seo_txtTags[]" id="tag_edit1"  value="" class="tag"/>
				<?php } ?>	
			</div>
		</div>
		</section> 
...
<script type="text/javascript" src="<?php echo image_url ?>js/article.js?<?php print date('his'); ?>"></script>
...
<script>
	$('.menu-list a').click(function(e){
		var prom=confirm("Are you sure wnt to go back?");
		if(prom==false){
			e.preventDefault();
		}
	});
