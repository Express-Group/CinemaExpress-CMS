...
echo '<h5 class="block-title margin-bottom-10" style="border-color: #fff;"><span style="margin-bottom: 0px;">'.$widgettitle.'</span></h5>';
...
if(count($widget_contents)>0)
{
	foreach($widget_contents as $get_content) // For Get Content Start Here 
	{
		...
		// Assign summary block starts here
		$colType = ($type==1) ? 2 : 3;
		$colTypeClass = ($type==1) ? 'col-lg-6 col-md-6 col-sm-6 col-xs-12 SundaySecondSplit' : 'col-lg-4 col-md-4 col-sm-4 col-xs-12 SundaySecondSplit';
		////////  For first Article  /////////
		...
		$show_simple_tab.= '<div class="'.$colTypeClass.'">';
		$show_simple_tab.= '<div class="hovereffect '.(($colType==2)? 'relative-img' :'').'"><a  href="'.$live_article_url.'" class="article_click" style="position:relative;">
		<img src="'.$dummy_image.'" data-src="'.$show_image.'" title = "'.$imagetitle.'" alt = "'.$imagealt.'"><img data-src="'.image_url.'uploads/user/imagelibrary/video-icon.png" src="'.image_url.'images/FrontEnd/images/lazy.png" class="GalleryIcon" style="position:absolute;"></a>';
		if($Rating!=0){
		...
		$show_simple_tab .= ($colType==2)? '<h4 class="subtopic"><span>'.$display_title.'</span></h4>': '<h4 class="subtopic">'.$display_title.'</h4>';
