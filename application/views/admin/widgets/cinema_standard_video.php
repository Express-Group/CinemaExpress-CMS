...
$show_simple_tab 	   .=	'<div style="padding-top:10px; color:#000"><fieldset class="FieldTopic"><legend class="topic"><a href="'.$widget_section_url.'">'.$widget_custom_title.'</a></legend></fieldset></div>';
...
if(count($widget_contents)>0)
{
	foreach($widget_contents as $get_content) // For Get Content Start Here 
	{
		...
		if($i==1)
		{
			...
			$show_simple_tab 	   .=	'<div class="news_widget" '.$widget_bg_color.'>';
		}
		
			if($i==1)
			{
				
			$show_simple_tab.= '<div class="news_left cinema_standard_video_inner">';
			...
			$show_simple_tab.= '<a  href="'.$live_article_url.'"><i class="fa fa-play video_iconsmall" aria-hidden="true"></i></a>';
			$show_simple_tab .='<div class="lead"><h4 class="subtopic">'.$display_title.'</h4></div>';
			$show_simple_tab.='<p class="summary custom_summary_video_widget">'.$summary.'</p>';
			$show_simple_tab.= '</div>'; //End news_widget 
			}
			else
			{
				if($i==2)
				{
				$show_simple_tab.= '<div class="news_right"><div class="news_right_block1">';
				}
						
			$show_simple_tab.= '<div class="news_right_block1_story1 cinema_standard_video_small">
			<a  href="'.$live_article_url.'" class="article_click"  >
			<img class="ImageBorder" src="'.$dummy_image.'" data-src="'.$show_image.'" title = "'.$imagetitle.'" alt = "'.$imagealt.'"><a  href="'.$live_article_url.'"><i class="fa fa-play video_iconsmall_1" aria-hidden="true"></i></a></a>';
			$show_simple_tab .='<div class="thumblead">'.$display_title.'';
				if($is_summary_required== 1)
				{
				$show_simple_tab.='<p class="summary custom_summary_video_widget">'.$summary.'</p>';
				}
			$show_simple_tab.='</div>';
			$show_simple_tab.= '</div>'; // end for  news_right_block1	
...
$show_simple_tab .=' <div class="custom_arrow"><a href="'.$widget_section_url.'">More '.strtolower($widget_custom_title).' >></a></div>';
