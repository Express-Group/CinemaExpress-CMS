...
$show_simple_tab.= ' <div class="btn-group" style="display:none;">
...
if(count($widget_contents) > 0)
{
	...
	foreach($widget_contents as $key_value => $get_content)
	{
		...
		if(trim($summary) == '') {
			...
			$bodycontent = substr($bodycontent, $headline_position);  
			}
		...
		if($count <= 3)
		{
			...
			$show_simple_tab.=	'<p class="post_time" style="color: black;">'.$post_time.' </p>';		
