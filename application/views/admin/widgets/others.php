...
$show_simple_tab .='<div class="row" '.$widget_bg_color.'>
				...
				$show_simple_tab .= ' <div class="slide others_slider others_slider_change" style="background-color: #f7eddf;">';
		     ...
		     {				
					foreach($widget_contents as $get_content)
					{
						...
						$show_image="";
							...
							$display_title = $alt_title =  strip_tags($display_title);
							$display_title = (strlen($display_title) > 50) ? substr($display_title , 0 , 47).'...' : $display_title;
						 	$display_title = '<a title="'.$alt_title.'"  href="'.$live_article_url.'" class="article_click" >'.$display_title.'</a>';
							$summary  = preg_replace('/<p[^>]*>(.*)<\/p[^>]*>/i', '$1',$custom_summary);
					     	//  Assign article links block ends hers
							if($m==1){
							 $show_simple_tab .= '<div class="other1">';
							}
							 $show_simple_tab .= '<a  href="'.$live_article_url.'">';
							 $show_image1 = str_replace("w600X390", "w600X300", $show_image);
							 $dummy_image1 = str_replace("nie_logo_600X390", "nie_logo_600X300", $dummy_image);
							 $show_simple_tab .= '<picture><source media="(min-width: 1551px)" srcset="'.$show_image1.'"><img src="'.$dummy_image1.'" data-src="'.$show_image.'" title = "'.$imagetitle.'" alt = "'.$imagealt.'"></picture>';
							 
							 $show_simple_tab .= '</a>';
						//	 $show_simple_tab .= '<h5 class="subtopic"><a  href="'.$sectionURL.'">'.ucfirst($section_name).'</a></h5>';				
									
										 $show_simple_tab .='<h3>'.$display_title.'</h3>';
