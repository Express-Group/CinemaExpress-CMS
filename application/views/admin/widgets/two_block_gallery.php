...
echo '<h5 class="block-title"><span>'.$widgettitle.'</span></h5>';
...
if(count($widget_contents)>0)
{
	foreach($widget_contents as $get_content) // For Get Content Start Here 
	{
		...
		if($count <= 2)
		{
			...
			$show_simple_tab.= '<div class="WidthFloat_L" style="background:#f7eddf;">'; 
			} 
			
		$show_simple_tab.= '<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 SundaySecondSplit">';
		$show_simple_tab.= '<div class="hovereffect"><a  href="'.$live_article_url.'" class="article_click" style="position:relative;">
		<img src="'.$dummy_image.'" data-src="'.$show_image.'" title = "'.$imagetitle.'" alt = "'.$imagealt.'"><img data-src="'.image_url.'uploads/user/imagelibrary/gallery-icon.png" src="'.image_url.'images/FrontEnd/images/lazy.png" class="GalleryIcon" style="position:absolute;">';
		if($Rating!=0){
