	...
	if(count($widget_contents) > 0 ){
		...
		foreach($widget_contents as $get_content)
		{
		   ...
		   $custom_summary  = preg_replace('/<p[^>]*>(.*)<\/p[^>]*>/i', '$1',$custom_summary);  //to remove first<p> and last</p>  tag
			...
			/*if($i == 1)
			{*/
				$show_simple_tab .='<div class="hpg_bg"><figure><a  href="'.$live_article_url.'"  class="article_click" >'.$img_icon.'<img src="'.$dummy_image.'" data-src="'.$show_image.'"></a></figure>';
				$show_simple_tab .='<h4>'.$display_title.'</h4>';
				if($is_summary_required== 1){	
				 $show_simple_tab .= '<p>'.$custom_summary.'</p></div>';
				}
			/*}	*/	
			$i =$i+1;							  
