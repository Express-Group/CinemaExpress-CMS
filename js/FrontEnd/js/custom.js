...
$(".widget-slideup-content").hover(function(){
	$(this).find('.widget-slideup-fixed').animate({top: "5%",height:"180px"},500);
	$(this).find('.widget-slideup-image').css( "opacity", "0.5" );
 }, function(){
	$(this).find('.widget-slideup-fixed').animate({top: "57%",height:"83px"},500);
	$(this).find('.widget-slideup-image').css( "opacity", "1" );
});  

$(".widget-slideup-content-one").hover(function(){
	$(this).find('.widget-slideup-fixed-one').animate({top: "24%",height:"200px"},500);
	$(this).find('.widget-slideup-image').css( "opacity", "0.5" );
  }, function(){
	$(this).find('.widget-slideup-fixed-one').animate({top: "75%",height:"90px"},500);
	$(this).find('.widget-slideup-image').css( "opacity", "1" );
}); 

	$('.search-icon').click(function(){
		if($('.search-inner').hasClass('inactive')){
			$('.search-inner').addClass('active').removeClass('inactive');
			$('.search-icon').css({'margin-left': '-15px','z-index': '25'});
			$('.search-form').show(600);
		}else if($('.search-inner').hasClass('active')){
			$('.search-inner').addClass('inactive').removeClass('active');
			$('#error_throw').html('');
			$('.search-form').hide(600);
		}
	});
